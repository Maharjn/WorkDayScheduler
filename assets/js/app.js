...
function createTimeBlock(hour) {
  ...
  const timeCol = $("<div>")
  timeCol.attr('class', 'hour col-2');

  timeCol.text(hour + ":00");
  // timeCol.addClass('hour');
  //timeCol.attr('class','hour');
  //timeCol.css("background-color","white")


  const textareaCol = $("<div >");
  textareaCol.attr('class', 'textarea-col col-8');
  const textarea = $('<textarea rows="3" cols="100">');
  textarea.attr('class', 'description col-12');
  ...
  const buttonCol = $("<div>");
  buttonCol.attr('class', 'save-button col-2');

  const saveButton = $('<button class="btn save-btn fas fa-save fa-2x">');
  //buttonCol.attr('class', 'saveBtn');
...
$(function () {
  ...
  // I should see 9am - 7pm timeblock
